<script lang="ts">
  ...
  onMount(async () => {
    ...
    onlyPrices = predicted_prices.predicted_prices.map(
      (price: any) => price.price
    );
    onlyDates = predicted_prices.predicted_prices.map(
      (price: any) => price.date
    );
...
<svelte:head>
  <title>{data.ticker_info.symbol} | stonk ai</title>
</svelte:head>
