...
>
  <div class="flex items-center gap-2">
    <div class="">
      {#if $isLoading}
        ...
        >
          <span class="loading loading-ring loading-lg text-primary-content"
          ></span>
        </div>
      {:else}
        <Icon
  ...
  {#if $isLoading}
    <div
      class="flex items-end gap-1 font-thin"
  ...
  {:else}
    <div class="font-thin">
      AI enhanced financial sentiment & analysis engine.
    </div>
  {/if}
  ...
  <form on:submit={handleSubmit} class="flex w-full max-w-sm flex-col gap-3">
    <div class="w-full">
      <label
        class="input input-bordered flex w-full items-center gap-2 focus-within:outline-none"
      >
        <input
          autofocus
          type="text"
      ...
      <div class="flex w-full max-w-lg items-center gap-2 pt-5">
        <div class="w-full">
          <button type="submit" class="btn btn-primary w-full">
            {#if $isLoading}
              <span class="loading loading-infinity loading-md scale-125"
              ></span>
            {:else}
              <div class="flex items-center gap-2">
